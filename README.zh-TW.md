...
- **修復 TcAdsHelper 載入**：重寫 `Initialize-TcAdsHelper`，使用 3 種回退策略（明確路徑 → 已載入組件 → 常見安裝路徑）確保 C# helper 可靠編譯。在 `Get-TcSymbols`、`Read-TcVariable`、`Write-TcVariable` 加入懶載入保護。`Connect-TcAds` 現在會在 helper 編譯失敗時發出警告。
